...
def job_red(it, periodo, max_time):
    np.random.seed(it)
...
def job_single_red(it, periodo, max_time):
    np.random.seed(it)
...
if __name__ == "__main__":
    ...
    with Pool(6) as p:
        estimations = np.array(
            p.starmap(job_red, zip(range(repetitions), periodogram_list, [max_time] * (repetitions))))
    print('|\n Done')
    # estimations = np.array([job(it, periodo) for it, periodo in zip(range(repetitions), periodogram_list)])
    end_time = time.time()
    print("Estimation time:", end_time - start_time)

    np.savetxt("saved_estimations/multivariate_triangle_red_" + str(max_time) + ".csv", estimations, delimiter=",")
...
# Single
    ...
    with Pool(6) as p:
        estimations = np.array(
            p.starmap(job_single_red, zip(range(repetitions), periodogram_list, [max_time] * (repetitions))))
    print('|\n Done')
    # estimations = np.array([job(it, periodo) for it, periodo in zip(range(repetitions), periodogram_list)])
    end_time = time.time()
    print("Estimation time:", end_time - start_time)

    np.savetxt("saved_estimations/multivariate_single_red_" + str(max_time) + ".csv", estimations, delimiter=",")